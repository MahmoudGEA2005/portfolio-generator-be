...
def register():
    ...
    if request.method == 'POST':
        ...
        return render_template('viewer.html', users=allUsers)
...
@jwt_required()
def create():
    # db.session.query(UsersPortfolios).delete()
    # db.session.query(Design1).delete()
    # db.session.commit()
    if request.method == "POST":
        ...
        allPortfolios = Design1.query.all()
        return render_template('portfolios.html', portfolios=allPortfolios)
...
def debug():
    data = UsersPortfolios.query.all()
